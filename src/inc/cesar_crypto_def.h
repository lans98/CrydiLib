...
#ifndef CRYDI3_CESAR_CRYPTO_DEF_H
#define CRYDI3_CESAR_CRYPTO_DEF_H 
...
template <class T>
class CesarCrypto : public Crypto<T> {
 public:
	CesarCrypto();
	CesarCrypto(const KeyList<T> &keys);
	CesarCrypto(const string &alpha);
	CesarCrypto(const string &alpha, const KeyList<T> &keys);
	~CesarCrypto() = default;
