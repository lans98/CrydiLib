...
string AfinCrypto<T>::Encrypt(string msg) {
	long alpha_size = this->alpha.size();
	for (string::iterator ch = msg.begin(); ch != msg.end(); ++ch) {
		string::size_type i = this->alpha.find(*ch);
		...
		T new_pos {
			Mod(this->keys[0] * conv<T>(index) + this->keys[1], conv<T>(alpha_size))
		};
		long new_pos_compat  = conv<long>(new_pos);
		*ch = this->alpha[new_pos_compat];
	}
	return msg;
}
...
string AfinCrypto<T>::Decrypt(string msg) {
	long alpha_size = this->alpha.size();
	T inv_key { FindModularInverse(this->keys[0], conv<T>(alpha_size)) };
	for (string::iterator ch = msg.begin(); ch != msg.end(); ++ch) {
		string::size_type i = this->alpha.find(*ch);
		...
		T new_pos {
			Mod(inv_key * (conv<T>(index) - this->keys[1]), conv<T>(alpha_size))
		};
		long new_pos_compat = conv<long>(new_pos);
		*ch = this->alpha[new_pos_compat];
	}
